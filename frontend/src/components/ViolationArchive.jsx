/**
 * ViolationArchive.jsx
 * SafeguardsIQ — Violation Archive with Date Filter
 * 30-day (Starter) / 90-day (Professional) plan limits
 * Place in: frontend/src/components/ViolationArchive.jsx
 */
...
const CAT_LABEL  = { ppe:"⛑️ PPE", pathway:"🚧 Pathway", unsafe:"⚠️ Unsafe", accident:"🚨 Accident", nearmiss:"❗ Near Miss" };
    ...
    <div style={{ fontFamily:"'Nunito',sans-serif", marginBottom:20 }}>
      <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:16, overflow:"hidden" }}>
        ...
        <div style={{ background:T.bg, padding:"16px 20px", borderBottom:`1px solid ${T.border}`,
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontSize:14, fontWeight:800, color:T.white }}>Violation Archive</div>
            <div style={{ fontSize:11, color:T.g1 }}>
              {maxDays}-day history · {plan.charAt(0).toUpperCase()+plan.slice(1)} Plan ·
              Filter, search and export violation records
            </div>
          </div>
        ...
        <div style={{ padding:16, background:T.card2, borderBottom:`1px solid ${T.border}` }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr 1fr", gap:10 }}>
            <div>
              <label style={{ fontSize:9, color:T.g1, display:"block", marginBottom:4, letterSpacing:1, fontWeight:700 }}>FROM DATE</label>
              <input type="date" value={filters.dateFrom} min={minDate} max={filters.dateTo}
                onChange={e => setF('dateFrom', e.target.value)}
                style={{ ...iStyle, width:"100%", boxSizing:"border-box" }}/>
            </div>
            <div>
              <label style={{ fontSize:9, color:T.g1, display:"block", marginBottom:4, letterSpacing:1, fontWeight:700 }}>TO DATE</label>
            ...
            <div>
              <label style={{ fontSize:9, color:T.g1, display:"block", marginBottom:4, letterSpacing:1, fontWeight:700 }}>CATEGORY</label>
              <select value={filters.category} onChange={e => setF('category', e.target.value)}
                style={{ ...iStyle, width:"100%", boxSizing:"border-box" }}>
                <option value="">All Categories</option>
                <option value="ppe">PPE Violation</option>
                <option value="pathway">Pathway</option>
                <option value="unsafe">Unsafe Act</option>
                <option value="accident">Accident</option>
                <option value="nearmiss">Near Miss</option>
              </select>
            </div>
            <div>
              <label style={{ fontSize:9, color:T.g1, display:"block", marginBottom:4, letterSpacing:1, fontWeight:700 }}>SEVERITY</label>
            ...
            <div>
              <label style={{ fontSize:9, color:T.g1, display:"block", marginBottom:4, letterSpacing:1, fontWeight:700 }}>CAMERA ID</label>
          ...
          <div style={{ display:"flex", gap:6, marginTop:10 }}>
            {[
              ...
              ...(maxDays >= 90 ? [{ label:"90 Days", days:90 }] : []),
            ].map(({ label, days }) => {
              const from = new Date(Date.now() - days * 86400000).toISOString().split('T')[0];
              const active = filters.dateFrom === from &&
            ...
            <div style={{ marginLeft:"auto", fontSize:11, color:T.g2, alignSelf:"center" }}>
              Max history: <span style={{ color:T.amber, fontWeight:700 }}>{maxDays} days</span>
              {maxDays < 90 && (
                <span style={{ color:T.g2 }}> · Upgrade to Professional for 90-day archive</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ padding:16 }}>
          {error && (
            <div style={{ padding:"10px 14px", background:`${T.red}10`,
              ...
              fontSize:12, color:T.red, marginBottom:12 }}>⚠ {error}</div>
          )}
          ...
          {loading ? (
            ...
            <div style={{ textAlign:"center", padding:"40px 0", color:T.g2 }}>
              ...
              <div style={{ fontSize:14, fontWeight:700, color:T.g1, marginBottom:4 }}>
                No violations found
              </div>
              <div style={{ fontSize:12 }}>Try adjusting the date range or filters</div>
            </div>
          ) : (
            <>
              {/* Column headers */}
              <div style={{ display:"grid",
                gridTemplateColumns:"140px 90px 80px 80px 1fr 120px 36px",
                ...
                return (
                  <div key={v.id || i}>
                    <div onClick={() => setExpanded(isExp ? null : i)}
                      ...
                      <div style={{ fontSize:10, fontWeight:700, color:cc }}>
                        {CAT_LABEL[cat] || cat}
                      </div>
                      <div style={{ fontSize:10, fontWeight:700, color:sc,
                        textTransform:"uppercase" }}>{sev}</div>
                      <div style={{ fontSize:12, color:T.white, fontWeight:600,
                        overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                        <span style={{ marginRight:6 }}>
                          {PPE_ICONS[v.violation_type] || "⚠️"}
                        </span>
                        {v.violation_type || "—"}
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                        <div style={{ flex:1, height:4, background:T.border, borderRadius:2 }}>
                          <div style={{ height:"100%", borderRadius:2,
                            ...
                            background:v.confidence >= 90 ? T.green
                              : v.confidence >= 70 ? T.amber : T.red }}/>
                        </div>
                        <span style={{ fontSize:10, color:T.g1,
                          fontFamily:"'DM Mono'", minWidth:32 }}>
                          {v.confidence || 0}%
                        </span>
                      </div>
                    ...
                    {/* Expanded detail */}
                    {isExp && (
                      <div style={{ margin:"0 0 8px 0", padding:16,
                        background:`${cc}06`, border:`1px solid ${cc}20`,
                        borderRadius:8, fontSize:12 }}>
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:10 }}>
                          <div>
                            <div style={{ fontSize:10, color:T.g2, marginBottom:3 }}>DESCRIPTION</div>
                            <div style={{ color:T.white }}>{v.description || "No description available"}</div>
                          </div>
                          <div>
                            <div style={{ fontSize:10, color:T.g2, marginBottom:3 }}>IMMEDIATE ACTION</div>
                            <div style={{ color:T.white }}>{v.immediate_action || "—"}</div>
                          </div>
                        </div>
                        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                          {[
                            { label:"Plant", val:v.plant_id },
                            { label:"Area",  val:v.area_id  },
                            { label:"Source",val:v.source   },
                          ].map(({label,val}) => val ? (
                            <div key={label} style={{ fontSize:10, color:T.g1,
              ...
              {totalPages > 1 && (
                <div style={{ display:"flex", justifyContent:"center",
                  ...
                  <button onClick={() => setPage(p => Math.max(1,p-1))}
                    ...
                    borderRadius:6, padding:"5px 12px", color:T.g1,
                    fontSize:12, cursor:page===1?"not-allowed":"pointer" }}>← Prev</button>

                  <span style={{ fontSize:12, color:T.g1 }}>
                    Page <span style={{ color:T.white, fontWeight:700 }}>{page}</span> of {totalPages}
                    <span style={{ color:T.g2 }}> ({total} records)</span>
                  </span>
                  ...
                  <button onClick={() => setPage(p => Math.min(totalPages,p+1))}
                    ...
                    background:"none", border:`1px solid ${T.border}`,
                    borderRadius:6, padding:"5px 12px", color:T.g1,
                    fontSize:12, cursor:page===totalPages?"not-allowed":"pointer" }}>Next →</button>
                </div>
              )}
            </>
          )}
